...
export default function WhiteboardPage() {
  ...
  const normalizeElement = (el: any) => {
    // Ensure all required Excalidraw properties exist
    return {
      ...
      ...(el.type === 'text' && { strokeColor: el.strokeColor || '#1a1a2e' }),
    };
  };
    ...
    setTimeout(() => {
      if (excalidrawAPIRef.current) {
        try {
          ...
          // Normalize all generated elements to ensure they have required properties
          const normalizedNewElements = elements
            .filter((el) => el && typeof el === 'object')
            .map((el) => normalizeElement(el));
          
          if (normalizedNewElements.length === 0) {
            console.warn('No valid elements after normalization');
            return;
          }

          const newElements = [...currentElements, ...normalizedNewElements];
          
          // Update scene with properly formatted elements
          ...
          if (excalidrawAPIRef.current.scrollToContent && newElements.length > 0) {
            try {
              excalidrawAPIRef.current.scrollToContent(normalizedNewElements, { animate: true });
            } catch (scrollErr) {
  ...
  return (
    <div className="flex gap-4 overflow-hidden" style={{ height: 'calc(100vh - 112px)' }}>
      ...
      {/* ── Main canvas area ────────────────────────────────────── */}
      <div className="flex-1 flex flex-col gap-0 min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white">
